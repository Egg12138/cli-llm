...
APP_TITLE = "egg-cli-llm"
APP_URL = "https://github.com/Egg12138/cli-llm"
EXTRA_HEADERS = {
...
def process_streamed_chunk(
    response, count_tokens: bool = False, extra_session_type: Optional[str] = None
) -> None:
...
def process_unstreamed_chunk(
    ...
    if extra_session_type == "Reasoning":
        print(
            f"{TIPF}@ {modelname} reasoning ========================================={RSTF}"
        )
...
def chat(
    ...
    # When JSON output is requested, ensure the prompt contains "json"
    # This is required by OpenAI/DeepSeek API when using response_format={'type': 'json_object'}
    # Without "json" in the prompt, the API returns a 400 error
    ...
    try:
        logger.info(
            f"🚀 Request to {model} ({'streaming output' if not no_stream else '⏺️ non-streaming'})"
        )
        if count_tokens:
        ...
        if not no_stream:
            ...
            response = client.chat.completions.create(**create_params)
            process_unstreamed_chunk(
                response, time.time() - start_time, count_tokens, "Reasoning"
            )
...
def get_sys_role(role: str) -> prompts.SystemPrompt:
    if role not in SYS_ROLES.keys():
        logger.warning(f"role is not predefined, use {default_role} as default")
        sys_role_marker = default_role
...
def chat_cli(
    ...
    if localtest:
        return test_result(client)

    chat(
        full_prompt, no_stream, model, sys_role, client, count_tokens, temp, json_output
    )
